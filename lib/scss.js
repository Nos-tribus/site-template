...
export const compileCssAndScss = async () => {
  ...
  const cssContent = []
  // cssContent.push(await compileScss('', {withMixins: true}))
  ...
  const compiledCss = await compileScss(cssContent.join('\n'), {withMixins: true})

  const beautifiedCss = cssbeautify(compiledCss, {
...
const compileScss = async (scssContent, options = {}) => {
  try {
    // const scssContent = scssFilePath ? fs.readFileSync(scssFilePath, 'utf8') : ''
    // const scssContent = scssFilePath ? fs.readFileSync(scssFilePath, 'utf8') : ''

    const mixins = []
    if (options.withMixins) {
      console.log('mixins')
...
const collectCssAndScssFiles = dirPath => {
  ...
  cssFiles.sort((a, b) => {
    const isMixinsA = /_mixins.*\.scss$/.test(a)
    const isMixinsB = /_mixins.*\.scss$/.test(b)

    if (isMixinsA && !isMixinsB) {
      return -1
    }
